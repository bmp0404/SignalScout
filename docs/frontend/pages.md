...
Public digest view showing the rotating "up next" list subscribers receive, plus auto-send status; the manual send and minimum-score control are operator-gated.
...
- `Digest()` — loads the upcoming digest via `api.upcomingDigest(offset)` through the shared `useAsyncData` hook, tracking a pagination `offsetRef` that the `NEW BATCH` button advances to the response's `next_offset` so each click cycles to a fresh batch of people; separately loads `api.overview()` + `api.digestSettings()` (`loadQualification`) to show "N of M discovered people currently qualify for the digest" (public, always visible); renders the auto-send status banner and each entry (rank, name, school/location, thesis, top signals, orbit/intro context, why-now, `ContactLinks`); exposes a `SEND NOW` button (`api.sendDigest()`) and a minimum-score number input + SAVE button (`saveMinScore` calls `api.updateDigestSettings({ min_score })` then reloads both the qualification count and the upcoming list), both wrapped in `AdminOnly` so only unlocked operators can trigger a send or move the threshold. Also renders `DigestSignup`.
