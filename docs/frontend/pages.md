...
Public digest view showing the rotating "up next" list subscribers receive, plus auto-send status; the manual send stays operator-gated, but the minimum-score control is fully public (the `PUT /api/digest/settings` endpoint isn't admin-gated — it only tunes eligibility, it doesn't spend credit or email anyone).
