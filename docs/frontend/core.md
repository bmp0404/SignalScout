...
- `App()` — renders the sticky header (title, tagline, tab nav) and the active tab's page component (`Discover`, `Backtest`, `Digest` with `operatorMode`, or `Pipeline`); on each tab change calls `api.pageView({ path, referrer })` (best-effort, errors are swallowed so analytics never break the UI).
