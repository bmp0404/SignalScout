...
- `build_router(container: Container) -> APIRouter` — constructs and returns the `/api`-prefixed router, defining a per-process in-memory rate-limit bucket and every route handler as a closure over `container`.
  ...
  - `update_digest_settings(payload)` — `PUT /api/digest/settings` — public/rate-limited, not admin-gated (tuning the score threshold doesn't spend provider credit or email subscribers, unlike generate/send); validates `min_score` (0–100 via `DigestSettingsRequest`) and persists it via `container.digest_settings.set_min_score(...)`, returning the saved value.
